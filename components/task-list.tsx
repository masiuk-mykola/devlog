...
function parseStatus(v: string | null): StatusFilter {
  return (STATUS_VALUES as readonly string[]).includes(v ?? "") ? (v as StatusFilter) : DEFAULT_STATUS;
}
function parseSort(v: string | null): SortKey {
  return (SORT_VALUES as readonly string[]).includes(v ?? "") ? (v as SortKey) : DEFAULT_SORT;
}
...
export function TaskList() {
  ...
  const setParam = (key: "status" | "sort" | "q" | "open", value: string | null, defaultValue?: string) => {
  ...
  const counts = useMemo(() => {
    const c = { all: 0, todo: 0, in_progress: 0, done: 0 };
  ...
  const filtered = useMemo(() => {
    ...
    else if (sort === "priority") sorted.sort((a, b) => PRIORITY_RANK[b.priority] - PRIORITY_RANK[a.priority]);
    return sorted;
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <Tabs value={status} onValueChange={(v) => setParam("status", v, DEFAULT_STATUS)}>
          <TabsList>
            <TabsTrigger value="all">All <span className="ml-1 text-[10px] opacity-60">{counts.all}</span></TabsTrigger>
            <TabsTrigger value="todo">Todo <span className="ml-1 text-[10px] opacity-60">{counts.todo}</span></TabsTrigger>
            <TabsTrigger value="in_progress">In progress <span className="ml-1 text-[10px] opacity-60">{counts.in_progress}</span></TabsTrigger>
            <TabsTrigger value="done">Done <span className="ml-1 text-[10px] opacity-60">{counts.done}</span></TabsTrigger>
          </TabsList>
        </Tabs>
        <div className="flex items-center gap-2">
          ...
          />
          <Select value={sort} onValueChange={(v) => setParam("sort", v, DEFAULT_SORT)}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {filtered.map((t) => <TaskRow key={t.id} task={t} onOpen={() => handleOpenTask(t.id)} />)}
        </div>
